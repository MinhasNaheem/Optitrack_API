...
namespace RigidBodySolver
{
    ...
    const int kDefaultSensorFusionCoefficient = 30;
    ...
    public:
            ...
            , Unique( true )
        {
            ...
            SensorFusionCoefficient = kDefaultSensorFusionCoefficient;
        }

        void  Save( Core::cIWriter *serial ) const;
        void  Load( Core::cIReader *serial );

        // Returns true if the differences are only for visual settings
        bool IdenticalSolverSettings( const cRigidBodySettings& other ) const;
